...
```bash
node .codex/skills/frontend/scripts/dev-server.mjs
```
...
```bash
node .codex/skills/impeccable/scripts/detector/detect-antipatterns.mjs .claude/preview/ --json
```
...
```
~/.codex/skills/frontend/preferences.json   # 用户偏好记忆（不提交 git）
```
