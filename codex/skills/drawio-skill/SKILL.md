...
## Review Loop + Change Log (评审循环 + 修改日志)

> 当用户指出图纸存在视觉问题（交叉/弯曲/穿插的线、布局不平衡等）时，进入评审循环。

### 评审流程

```
导出 PNG → 视觉评审 → 输出问题清单 → 修复 → 重新导出 → 再次评审 → ... → 通过
                ↑______________________________________________________|
```

1. **导出 PNG**：用 draw.io CLI 导出当前版本的 PNG（`--width 2000`，no `-e`）
2. **视觉评审**：读取 PNG，从以下维度分析：
   - **连线交叉**：不同连线之间是否有不必要的交叉？
   - **连线弯曲**：连线是否有过多不必要的弯曲？
   - **连线穿插**：连线是否穿过了不相关的节点或分组框？
   - **节点重叠**：节点之间是否有重叠？
   - **布局平衡**：整体布局是否平衡？是否有某侧过于拥挤？
   - **标签可读性**：连线标签是否清晰可读？
3. **输出问题清单**：按严重程度（高/中/低）分类，给出修复建议
4. **修复**：根据问题清单修改 `.drawio` XML
   - 单元素修改 → 编辑现有 XML in place
   - 布局级修改 → 重新生成完整 XML
5. **重新导出 PNG**：修复后重新导出
6. **再次评审**：直到所有高优先级问题解决，用户确认 "通过/LGTM"
7. **记录修改日志**：写入 `output/[项目名]/drawio/[项目名]-drawio-changelog.md`

**安全阀**：最多5轮评审循环，超过后建议用户在 draw.io Desktop 中手动微调。

### 修改日志格式

写入路径：`output/[项目名]/drawio/[项目名]-drawio-changelog.md`

```markdown
## DrawIO 修改日志 — [项目名]

### 第1轮 (YYYY-MM-DD HH:MM)
| 图纸 | 问题类型 | 位置 | 严重程度 | 修复方式 | 验证结果 |
|------|---------|------|---------|---------|---------|
| architecture | 连线穿插 | xxx→yyy | 高 | 调整节点位置，添加waypoints | ✅ |
| flowchart | 连线弯曲 | xxx→yyy | 中 | 重构分支节点布局 | ✅ |

### 第2轮 (YYYY-MM-DD HH:MM)
| 图纸 | 问题类型 | 位置 | 严重程度 | 修复方式 | 验证结果 |
|------|---------|------|---------|---------|---------|
| swimlane | 布局不平衡 | LLM解读层 | 高 | 收窄泳道宽度 | ✅ |

### 评审结论
- 第N轮：所有高优先级问题已解决，用户确认通过
```

### 评审 Agent Prompt（完整版 — v2.0 强化版）

> 基于本次评审经验教训，新增：决策节点出口方向检查、回路检查、修复后强制再次评审。

```
你是一个 drawio 图纸评审专家。你的任务是基于【PRD 业务规范】+【DrawIO 工具规范】，
对图纸进行系统化的评审。本次评审是第[N]轮，必须逐条检查所有维度，不得遗漏。

## 输入信息

1. **PRD 业务规范**（业务层，定义"画什么"）：
   - 分层定义：参见 PRD `standards/diagram-spec-v2.md` §1.1
   - 连线规则：实线=主动调用，虚线=数据回传/配置读取
   - 禁止跨层越级连线
   - Ontology 配置归属数据层子分组

2. **DrawIO 工具规范**（工具层，定义"怎么画"）：
   - XML 结构：`id="0"` + `id="1"` root cells
   - 容器：swimlane（层级）→ swimlane（分组）→ vertex（节点）
   - 节点 style：参见 `references/architecture-rules.md` §2
   - 连线 style：参见 `references/architecture-rules.md` §3
   - 像素间距：层级间≥80px，分组间≥40px，节点间≥20px

3. **评审对象**：
   - .drawio XML 文件路径：[path]
   - PNG 导出图路径：[path]（必须读取PNG进行视觉评审）
   - 蓝图文件路径：[path]（用于对照）
   - 轮次：[第N轮]

## 强制规则（🚨 评审前必读）

1. **必须读取 PNG 进行视觉评审** — 不能只读XML，必须读取导出的PNG图片
2. **必须逐条检查所有维度** — 不能跳过任何一条
3. **必须检查每个 decision 节点的出口方向** — 出口必须分散（左/下/右），不能堆叠
4. **必须检查每条连线的路径** — 是否有不必要的弯曲、交叉、穿插、绕远
5. **必须检查回路** — 是否有绕远的回路（应该最短路径返回）
6. **修复后必须再次评审** — 修复完成后，重新导出PNG，再次执行完整评审

## 评审维度（按优先级排序，必须逐条勾选）

### P0 — 规范一致性（业务层）— 最高优先级

- [ ] **分层正确**：应用层→中台层→数据层，无越级
- [ ] **Ontology归属**：Ontology配置归属数据层子分组，不独立分层
- [ ] **线型正确**：实线=调用，虚线=回传/配置，无混用
- [ ] **连线方向**：自上而下=实线，自下而上=虚线
- [ ] **节点命名**：中文业务名词，≤12字，缩写补充全称

### P1 — 视觉质量（工具层）— 高优先级

**1.1 连线交叉检查**
- [ ] 是否有不必要的连线交叉？（正常交叉不可避免，但应尽量减少）
- [ ] 交叉点是否清晰可辨？（不能有多条线重叠交叉）

**1.2 连线弯曲检查**
- [ ] 是否有过多不必要的弯曲？（每个折弯必须有原因）
- [ ] 连线路径是否尽量短直？（避免绕远）
...
**1.4 决策节点出口方向检查（🚨 重点）**
- [ ] 每个decision节点的出口是否分散？（左/下/右，不能都朝一个方向）
- [ ] 出口标签是否清晰标注？（是/否、策略人员/客服/管理层等）
- [ ] 异常分支（超时/失败）是否朝左侧或右侧延展？

**1.5 回路检查（🚨 重点）**
- [ ] 是否有回路？（如"用户是否追问→是→回到前面"）
- [ ] 回路是否绕远？（应该最短路径返回）
- [ ] 回路是否与主流程交叉？

**1.6 布局平衡检查**
- [ ] 是否有某侧过于拥挤？
- [ ] 节点间距是否符合规范？（层级间≥80px，分组间≥40px，节点间≥20px）
- [ ] 画布大小是否足够？（节点没有被截断）
...
- [ ] 所有 edge 是否有 `<mxGeometry relative="1" as="geometry" />`？
- [ ] 节点坐标是否对齐到 10 的倍数？
- [ ] 容器嵌套是否正确？（层级→分组→节点）
- [ ] 连线是否使用 `orthogonalEdgeStyle`？（禁止斜线）
- [ ] 是否有 dangling edge（source/target指向不存在的节点）？
- [ ] 是否有重复 ID？
...
| 错误类型 | 示例 | 正确做法 |
|---------|------|---------|
| 决策节点出口堆叠 | 两个出口都朝下 | 分散到左/下/右 |
| 连线直穿节点 | 从A到B的线穿过C | 添加waypoints绕行 |
| 回路绕远 | "是"的回路绕了一大圈 | 最短路径返回 |
| 异常分支不标注 | 超时/失败没有标签 | 必须标注"超时"/"失败" |
| 节点重叠 | 两个节点部分重叠 | 增大间距 |
...
| 2 | 坐标对齐 | ✅/❌ | ... |
| 3 | 容器嵌套 | ✅/❌ | ... |
| 4 | orthogonalEdgeStyle | ✅/❌ | ... |
| 5 | dangling edge | ✅/❌ | ... |
| 6 | 重复ID | ✅/❌ | ... |

### 问题清单（按严重程度排序）

| 优先级 | 问题类型 | 位置 | 严重程度 | 描述 | 修复建议 | PNG坐标 |
|--------|---------|------|---------|------|---------|---------|
| P0/P1/P2 | xxx | xxx→yyy | 高/中/低 | ... | ... | (x,y) |

### 修复后再次评审要求

**修复完成后，必须：**
1. 重新导出 PNG
2. 重新执行完整评审（本Prompt的所有维度）
3. 对比修复前后的PNG，确认问题已解决
4. 如果仍有问题，继续修复并再次评审
5. 最多5轮，超过后建议用户在 draw.io Desktop 中手动微调

### 评审结论

- **通过条件**：所有 P0 和 P1 项均为 ✅，P2 项可容忍少量 ❌
- **不通过条件**：任何 P0 项为 ❌，或超过3个 P1 项为 ❌
- **本轮结论**：通过 / 不通过，需修复 [N] 个问题
```
...
const reviewResult = await agent(
  `调用评审 Agent（第${round}轮），读取以下文件进行评审：
   - .drawio XML: ${drawioPath}
   - PNG: ${pngPath}（必须读取PNG进行视觉评审）
   - PRD 规范: standards/diagram-spec-v2.md
   - DrawIO 规范: references/architecture-rules.md
   评审维度：P0(规范一致性) + P1(视觉质量，含决策节点出口方向/回路检查) + P2(技术正确性)
   强制规则：
   1. 必须读取PNG进行视觉评审
   2. 必须逐条检查所有维度
   3. 必须检查每个decision节点的出口方向
   4. 必须检查每条连线的路径
   5. 必须检查回路是否绕远
   6. 修复后必须再次评审（重新导出PNG）`,
  {label: `drawio-review:round${round}`, phase: 'Review'}
)
```

### 修复 Agent Prompt（完整版 — v2.0 强化版）

```
你是一个 drawio XML 修复专家。你的任务是根据评审报告，修复 .drawio XML 文件中的问题。
...
2. **当前 .drawio XML 文件路径**：[path]

3. **修复策略**：
   - 单元素修改 → 编辑现有 XML in place（编辑 mxCell 的 style/geometry）
   - 布局级修改 → 重新生成完整 XML（重写整个 diagram）
...
2. **连线路径优化**：
   - 添加 waypoints（`<Array as="points">`）
   - 先垂直再水平（或先水平再垂直）
   - 避免斜线（必须使用正交连线）
   - 回路最短路径

3. **节点位置调整**：
   - 增大间距（同组≥20px，跨组≥200px）
   - 所有坐标对齐到10的倍数
   - 避免节点重叠
...
1. 修改后的 .drawio XML 文件
2. 修改日志条目（写入 changelog）
3. 验证结果（导出 PNG 并确认问题已解决）
4. 修复前后的 PNG 对比（可选）
```
...
const fixResult = await agent(
  `调用修复 Agent（第${round}轮），根据评审报告修复 .drawio XML：
   - 评审报告: ${reviewReport}
   - .drawio 文件: ${drawioPath}
   修复策略：
   1. 决策节点出口方向分散（左/下/右）
   2. 连线路径优化（添加waypoints，避免斜线）
   3. 回路最短路径
   4. 修复后重新导出PNG验证`,
  {label: `drawio-fix:round${round}`, phase: 'Fix'}
)
```
3. 蓝图规范
