---
name: memory-bridge
description: 跨项目记忆桥接 — 双向能力：①同步沉淀（从各项目 .claude/memory/ 拉取 bridge:true 记忆→分类沉淀到记忆中枢→更新全局索引）；②经验检索（按关键词/分类搜索已沉淀的跨项目经验，追溯来源项目）。
---

# Memory Bridge Skill

跨项目记忆同步工具。从各项目的 `.claude/memory/` 拉取长期记忆，结构化沉淀到日常仓库 `03_Knowledge/记忆中枢/`，并更新全局 `~/.claude/MEMORY.md` 索引。
...
首次运行前需创建配置文件 `~/.claude/memory-bridge-config.json`：
...
2. `projects` 缺失 → 扫描 `~/.claude/projects/` 下所有哈希目录，通过 git remote 或目录名推断项目名

## 执行流程

### 1. 确认项目列表 + 锁定工作目录

读取 `~/.claude/memory-bridge-config.json`。若文件不存在，使用自动探测 fallback 并提示用户创建配置文件。
...
- **bridge 字段缺失 → 视为 true**（遵循 CLAUDE.md bridge 默认值规则）
...
6. **supersedes 记录**：合并后将旧文件的 `name` 写入合并笔记的 `supersedes` 字段（遵循 CLAUDE.md supersedes 记忆合并规则）
...
- `source_memory`：原始 .claude/memory/ 文件路径（可溯源到具体踩坑记录）
- `bridge_sync`：最后同步时间
...
|---|---------|------|---------|---------|--------|
| 1 | Claude-Code工具踩坑 | 通用经验 | AI Worker Hub, PRD项目 | 2026-06-08 | 🟢 高 |
...
- 原始记忆文件：.claude/memory/lessons/claude-code-param-loss.md
```

**5. 可选：打开来源追溯**

用户要求查看原始记忆文件时，用 Read 读取 `source_memory` 指向的 `.claude/memory/` 文件，展示原始踩坑上下文。
...
- 权限：读取其他项目 `.claude/memory/` 通过 Bash cat 命令
